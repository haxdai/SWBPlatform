 ...
 */
//TODO: Review this class to make it SOLID if needed. Code is used in SWBSocial.
public class SparqlProcessor {
    ...
    public List<ParamNode> getSQLParams() {
        return sqlparams;
    }
    ...
    public String getIternalQuery(String modelid) {
        ...
        if (querytype.equals("select")) {
            ...
            try {
                ...
                Logger.getLogger(SparqlProcessor.class.getName()).log(Level.SEVERE, null, e);
            }

            if (dbtype.equals(SWBUtils.DB.DBTYPE_MySQL)) {
    ...
    private String getTextNode(Tree t, String []ruleNames) {
        ...
        if (t instanceof RuleNode) {
            node = "rn:" + ruleNames[((RuleNode) t).getRuleContext().getRuleIndex()];
        } else if (t instanceof ErrorNode) {
            node = "en:" + t.toString();
        } else if (t instanceof TerminalNode) {
            node = "tn:" + ((TerminalNode) t).getSymbol().getText();
        }
        return node;
    }
    ...
    private boolean processNode(Tree t, String ruleNames[]) {
        ...
        if ((t instanceof RuleNode)) {
            ...
            if (validatePath(t, ruleNames, new String[]{"rn:selectClause"})) {
                if ("tn:select".equalsIgnoreCase(node)) {
                    querytype = "select";
                } else {
                ...
                if ("tn:DESC".equalsIgnoreCase(node)) {
...
class QueryNode {
    ...
    public boolean isVar() {
        return (nodeType.equals(VAR));
    }

    public boolean isIRI() {
        return (nodeType.equals(IRI));
    }

    public boolean isString() {
        return (nodeType.equals(STRING));
    }
    ...
    public void addInternalVar(String var, boolean optional) {
        if (isVar() && !(optional && !internalVars.isEmpty())) {
            internalVars.add(var);
        }
    }

    public String getLastInternalVar() {
        if (isVar() && !internalVars.isEmpty()) {
